...
すべての料理行・季節限定の一杯・写真ビューア内に**「イメージ写真（生成）」**を明記し、実在店舗・実在メニューの写真としては扱いません。ビューアは前後の料理へ移動でき、切替時は旧写真をいったん消して、濃紺の背景に星の紋を一度だけ出してから新しい写真と文字をふわっと定着させます。右下の「写真を閉じる」は、下線が一度だけ伸びる主操作です。自動送りはせず、モーション停止時は即時に切り替わります。
